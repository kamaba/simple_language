...
namespace SimpleLanguage.Compile.CoreFileMeta
{
    public class FileMetaParamterDefine : FileMetaBase
    {
        ...
        public bool ParseBuildMetaParamter(List<Node> inputNodeList )
        {
            ...
            if (nameNode == null)
            {
                Debug.WriteLine("Error 没有找到该定义名称 必须使用例: X = 101; 的格式");
                return false;
            }
    ...
    public class FileMetaMemberFunction : FileMetaFunction
    {
        ...
        public bool ParseFunction(List<Node> nodeList)
        {
            ...
            while (addCount < nodeList2.Count)
            {
                ...
                else
                {
                        ...
                        || token.type == ETokenType.Extern )
                    {
                        ...
                        {
                            isError = true;
                            Debug.WriteLine("Error 解析过了一次权限!!");
                        }
                    }
                    else if (token.type == ETokenType.Override)
                    {
                        ...
                        {
                            isError = true;
                            Debug.WriteLine("Error 解析过了一次Override!!");
                        }
                        virtualToken = token;
                    }
                    ...
                    else if (token.type == ETokenType.Get)
                    {
                        ...
                        {
                            isError = true;
                            Debug.WriteLine(" Error 解析类型多个get");
                        }
                        getToken = token;
                    }
                    else if (token.type == ETokenType.Set)
                    {
                        ...
                        {
                            isError = true;
                            Debug.WriteLine(" Error 解析类型多个set");
                        }
                        setToken = token;
                    }
                    ...
                    else if (token.type == ETokenType.Interface)
                    {
                        ...
                        {
                            isError = true;
                            Debug.WriteLine("Error 解析interface已使用过一次，不允许重复使用!!");
                        }
                    ...
                    else if( token.type == ETokenType.Final )
                    {
                        ...
                        {
                            isError = true;
                            Debug.WriteLine(" Error 解析类型多个final");
                        }
                        finalToken = token;
                    }
                    else
                    {
                        isError = true;
                        Debug.WriteLine("Error 有其它未知类型在class中");
                        break;
                    }
                }
            }
            if(funNameNode == null )
            {
                Debug.WriteLine("Eror 没有找到合适的函数类型: 位置: " + nodeList[0].token?.ToLexemeAllString());
                return false;
            }
            ...
            if( isError )
            {
                Debug.WriteLine("ParseFunction 解析函数");
            }
