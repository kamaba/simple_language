#!
 * Std/Isolate/ReceivePort.sl
 * 端口接收端：消息队列 + 阻塞 / 非阻塞取消息 + Stream 风格 listen。
 * ReceivePort 本身不可跨 isolate 发送（端口属本 isolate 所有）；
 * 跨 isolate 传递请使用 sendPort（SendPort 视图）。
!#

public class ReceivePort extends Object
{
     ...
     * handler 建议签名 Func<void, object>（收一条消息调用一次）；
     * 端口关闭且消息耗尽后分发协程自动退出。
    !#
