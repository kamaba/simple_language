...
# 绑定失败（库未注册 / 符号缺失 / sig 非法）时回退执行函数本体（SL 纯实现
# fallback，见本文件后半的 xxxFallback 私有静态实现）。
...
public class Mathd
{
    ...
    @DllStaticImport( "math_lib", "mathd_dot3" )
