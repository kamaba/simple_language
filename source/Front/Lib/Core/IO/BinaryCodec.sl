...
#
# P2 范围：闭包注入版 Codec<object, ByteBuffer>。encode/decode 逻辑由调用方
# 提供（System.Buffer 风格的字段序读写由调用方手写，与 ProtoBuf 共用
# varint 原语走 ByteBuffer system method）。
#
# 已知偏差（详见设计文档 §17 实现注记）：
...
#     反射 syscall 落地后补。
#   - 设计稿 extends Codec<object, ByteBuffer>：object/ByteBuffer 均为具体类
...
public class BinaryCodec extends Codec<object, ByteBuffer>
{
    # encode 闭包：object → ByteBuffer（字段序字节布局由调用方手写）
    Function _encoderFn = null
    # decode 闭包：ByteBuffer → object
    ...
    override public ChunkedConversionSink<object> startChunkedConversion( ChunkedConversionSink<ByteBuffer> sink )
    {
        Converter<object, ByteBuffer> enc = this.encoder
        ret enc.startChunkedConversion( sink )
    }

    override get Converter<object, ByteBuffer> encoder()
    {
        Function fn = this._encoderFn
        var c = _FnConverter<object, ByteBuffer>( fn )
        ret c
    }

    override get Converter<ByteBuffer, object> decoder()
    {
        Function fn = this._decoderFn
        var c = _FnConverter<ByteBuffer, object>( fn )
        ret c
    }
}
