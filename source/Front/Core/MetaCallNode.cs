    ...
    public sealed class MetaCallNode
    {
        ...
        bool CreateCallNode()
        {
            ...
            else if (etype == ETokenType.Base)
            {
                ...
                if (owningMc == null)
                {
                    Log.AddMetaCoreLog(LID.ShowExtendMessage, "Error base requires class context.");
                    return false;
                }
            ...
            else if (etype == ETokenType.Local)
            {
                ...
                {
                    Log.AddMetaCoreLog(LID.MetaCoreAssertShowMessage, m_Token, "local.鍙兘鍦ㄩ浣?");
                    return false;
                }
            }
        ...
        private bool TryBuildDataToStringSystemCall(string inputname)
        {
            ...
            if (frontNode.metaVariable != null)
            {
                ...
                callerNode = MetaVisitNode.CreateByVariable(frontNode.metaVariable, callerMt != null ? new MetaType(callerMt) : null);
            }
            else
            {
        ...
        public bool GetFirstNode(string inputname, MetaBase mb , int count)
        {
            ...
            else
            {
                ...
                else if (me != null)
                {
                    if (m_IsFunction)
                    {
                        Log.AddMetaCoreLog(LID.ShowExtendMessage, "Error data 不支持函数调用: " + me.allName);
                        return false;
                    ...
                    else
                    {
                        ...
                        {
                            Log.AddMetaCoreLog(LID.ShowExtendMessage, $"Error data '{me.allName}' does not have member variable '{inputname}'");
                            return false;
                        }
                    }
                }
