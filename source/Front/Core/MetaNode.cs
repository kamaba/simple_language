...
namespace SimpleLanguage.Core
{
    ...
    public class MetaNode
    {
        ...
        public MetaNode(MetaClass mc)
        {
            ...
            {
                Log.AddMetaCoreLog(LID.Unknown, $"已有该模板数量的类{mc.name} :{mc.metaTemplateList.Count}");
                return;
            }
        ...
        {           
            ...
            {
                Log.AddMetaCoreLog(LID.Unknown, "添加命名空间");
            }
        ...
        public MetaNode AddMetaClass( MetaClass mc )
        {
            ...
            if (m_ChildrenMetaNodeDict.ContainsKey(mc.name))
            {
                ...
                if(node != null )
                {
                    ...
                    {
                        Log.AddMetaCoreLog(LID.Unknown, "添加add metaclass metanode节点有问题! 有重复:" + mc.allClassName );
                        return null;
                    }
                    else
                    {
        ...
        public bool AddMetaNode( MetaNode mn )
        {
            ...
            {
                Log.AddMetaCoreLog(LID.Unknown, $"Parent:{this.GetAllName()} Current:{ mn.name } 添加 add metanode metanode节点有问题! 有重复");
                return false;
            }
