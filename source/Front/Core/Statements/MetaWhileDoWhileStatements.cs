    ...
    public sealed class MetaForStatements : MetaStatements
    {
        ...
        private void Parse()
        {
            ...
            if ( m_IsForIn )
            {
                ...
                {
                    Log.AddMetaCoreLog( LID.AutoMetaWhileDoWhileStatementsL68, "Error for in express后边没有表达式!!");
                }
                ...
                {
                    Log.AddMetaCoreLog(LID.AutoMetaWhileDoWhileStatementsL98, "Error For in 表达式，应该是个数组形式");
                    return;
                }
                if( mcallEn != null )
                {
                ...
                if ( m_FileMetaKeyForSyntax.fileMetaClassDefine is FileMetaDefineVariableSyntax fmcd )
                {
                    ...
                    if (dmv != null)
                    {
                        Log.AddMetaCoreLog(LID.AutoMetaWhileDoWhileStatementsL135, "Error 在 for .. in 中，不允许从for 外边定义遍历变量!!");
                        return;
                    }
                    else
                    {
                ...
                else if( m_FileMetaKeyForSyntax.fileMetaClassDefine is FileMetaCallSyntax fmcs )
                {
                    ...
                    if (dmv != null)
                    {
                        Log.AddMetaCoreLog(LID.AutoMetaWhileDoWhileStatementsL149, "Error 在 for .. in 中，不允许从for 外边定义遍历变量!!");
                        return;
                    }
                    else
                    {
            ...
            else
            {
                ...
                if (m_ForIterateVariable == null)
                {
                    Log.AddMetaCoreLog(LID.AutoMetaWhileDoWhileStatementsL233, "Error 没有找到相应的变量!!");
                }
                m_ThenMetaStatements.UpdateMetaVariableDict(m_ForIterateVariable);
