                ...
                if (!_cts.IsCancellationRequested)
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            //logger.Log((int)LID.ShowExtendMessage, message);
        ...
        public async Task RunPhasesAsync(IEnumerable<Func<CancellationToken, Task>> phases)
        {
            foreach (var phase in phases)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Abort((int)LID.ShowExtendMessage, "Unhandled exception in phase: " + ex.Message);
                    break;
                }
            }
        }
            ...
            using (var sem = new SemaphoreSlim(options.MaxDegreeOfParallelism))
            {
                foreach (var worker in workers)
                {
                    ...
                    var t = Task.Run(async () =>
                    {
                        ...
                        {
                            Abort((int)LID.ShowExtendMessage, "Unhandled worker exception: " + ex.Message);
                        }
                        finally
                        {
                            sem.Release();
