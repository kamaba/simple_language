...
namespace SimpleLanguage.Core
{
    ...
    public sealed class MetaCallNode
    {
        ...
        public MetaModule m_MetaMoule { get; private set; } = null;
        public MetaNamespace m_MetaNamespace { get; private set; } = null;
        ...
        public bool ParseNode(AllowUseSettings _auc)
        {
            ...
            if (m_FileMetaCallSign != null)
            {
                ...
                else if (m_FileMetaCallSign.token.type == ETokenType.And)
                {
                    ...
                    Debug.Write("Error MetaStatements Parse  不允许使用其它连接符!!");
                    return false;
                }
                else
                {
            ...
            {
                Debug.Write("Error 定义原数据为空!! " + m_Token.ToLexemeAllString());
            }
            if (m_FileMetaCallNode.fileMetaParTerm != null && !m_IsFunction )
            {
                ...
                if (firstNode == null)
                {
                    Debug.Write("Error 不能使用输入()中的内容 0号位的没有内容!!");
                }
                else
                {
        ...
        bool CreateCallNode()
        {
            ...
            {
                Debug.Write("Error 前边节点没有发现MetaBase!!");
                return false;
            }
            ...
            else if( etype == ETokenType.Global )
            {
                if (isFirst)
                {
                    if (m_IsFunction)
                    {
                        Debug.Write("Error 不允许global的函数形式!!");
                    }
                    else
                    {
                ...
                {
                    Debug.Write("Error  不允许 使用global 只有第一位置可以使用This关键字" + m_Token.ToLexemeAllString());
                }
            }
            else if (etype == ETokenType.This)
            {
                ...
                {
                    Debug.Write("Error 不允许在成员变量中使用this关键字" + m_Token.ToLexemeAllString());
                }
                if (this.m_AllowUseSettings.parseFrom == EParseFrom.InputParamExpress)
                {
                    Debug.Write("Error 不允许在输入变量中中使用this关键字" + m_Token.ToLexemeAllString());
                }
                ...
                if (isFirst)
                {
                    if (m_IsFunction)
                    {
                        Debug.Write("Error 不允许this的函数形式!!" + m_Token.ToLexemeAllString() );
                    }
                    else
                    {
                ...
                {
                    Debug.Write("Error 只有第一位置可以使用This关键字" + m_Token.ToLexemeAllString());
                }
            }
            else if (etype == ETokenType.Base)
            {
                if (this.m_AllowUseSettings.parseFrom == EParseFrom.MemberVariableExpress)
                {
                    Debug.Write("Error 不允许在成员变量中使用base关键字" + m_Token.ToLexemeAllString());
                }
                if (this.m_AllowUseSettings.parseFrom == EParseFrom.InputParamExpress)
                {
                    Debug.Write("Error 不允许在输入变量中中使用base关键字" + m_Token.ToLexemeAllString());
                }

                MetaClass parentClass = m_OwnerMetaClass.parentNode as MetaClass;
                if ( parentClass == null )
                {
                    Debug.Write("Error 使用base没有找到父节点!!");
                    return false;
                }
                ...
                if (isFirst)
                {
                    if (m_IsFunction)
                    {
                        Debug.Write("Error 不允许base的函数形式!!");
                    }
                    else
                    {
                ...
                {
                    Debug.Write("Error 只有第一位置可以使用base关键字" + m_Token.ToLexemeAllString());
                }
            }
            ...
            else if (etype == ETokenType.Identifier)
            {
                ...
                {
                    // Class1. ns. Int32[]
                    tempMetaBase = GetFirstNode(name, m_OwnerMetaClass );
                }
                else
                {
                    if (frontCNT == ECallNodeType.NamespaceName)
                    {
                        ...
                        {
                            Debug.Write("Error 在Namespace下不存在在方式!");
                        }
                    }
                    ...
                    else if (frontCNT == ECallNodeType.ClassName)
                    {
                        ...
                        if (tmb == null)
                        {
                            ...
                            if(mmv != null )
                            {
                                if( !mmv.isStatic )
                                {
                                    Debug.Write("Error 调用非静态成员，不能使用Class.Variable的方式!");
                                    return false;
                                }
                                m_MetaVariable = mmv;
                                tmb = mmv;
                            ...
                            if(tmb == null )
                            {
                                ...
                                if (mmf != null)
                                {
                                    ...
                                    {
                                        Debug.Write("Error 不允许使用构造函数" + m_Token.ToLexemeAllString());
                                        return false;
                                    }
                                    m_MetaFunction = mmf;
                                    tmb = mmf;
                    ...
                    else if (frontCNT == ECallNodeType.MemberDataName)
                    {
                        ...
                        if (retmmd == null)
                        {
                            Debug.Write($"Error 没有找到{name} 的MetaData数据!");
                            return false;
                        }
                    ...
                    else if( frontCNT == ECallNodeType.EnumName )
                    {
                        ...
                        else
                        {
                            ...
                            if (mme != null)
                            {
                                ...
                                {
                                    Debug.Write("不能使用Enum.metaVariable(2) 这样的格式!");
                                    return false;
                                }
                                else
                                {
                            ...
                            {
                                Debug.Write("Error 不能使用Enum.xxxx未发现后续!");
                                return false;
                            }
                        }
                    }
                        ...
                        if(tempMetaBase2 == null )
                        {
                            ...
                            else
                            {
                                ...
                                if (mmf != null )
                                {
                                    if( mmf.isStatic )
                                    {
                                        Debug.Write("Error 该位置为非静态函数查找 但找到了静态函数!!" + m_FileMetaCallNode.token?.ToAllString());
                                        return false;
                                    }
                    ...
                    {
                        Debug.Write("Error 暂不支持上节点的类型: " + frontCNT.ToString());
                    }
                }
            }
            ...
            {                
                ...
                else if ( m_MetaClass != null )
                {
                    ...
                    if (curmc.isTemplateClass)
                    {
                        ...
                        if (tmitc != null && tmitc.metaTemplateParamsList.Count > 0)
                        {
                            ...
                            if (mtc == null)
                            {
                                Debug.Write("Error 在MetaCommon没有找到相关的生成类");
                                return false;
                            }
                            MetaMemberFunction mmf = mtc.GetMetaMemberFunctionByNameAndInputParamCollect("_init_", m_MetaInputParamCollection);
                            if (mmf == null)
                            {
                                Debug.Write("Error 没有找到相关的_init_类!!");
                                return false;
                            }
                            m_MetaClass = mtc;
                            m_MetaFunction = mmf;
                    ...
                    else
                    {
                        ...
                        if (mmf == null)
                        {
                            Debug.Write("Error 没有找到 关于类中" + curmc.allName + "的_init_方法!)");
                            return false;
                        }
                        m_MetaClass = curmc;
                        m_MetaFunction = mmf;
                        ...
                        if (m_FileMetaCallNode.fileMetaBraceTerm != null)  //可以使用  ArrClass(){ x = ??} 的方式
                        {
                            ...
                            {
                                Debug.Write("Error 在InputParam 里边，构建函数，只允许 使用ClassName() 的方式, " +
                    ...
                    {
                        Debug.Write("Error 当前位置不允许有函数调用方式使用!!!" + m_Token?.ToLexemeAllString());
                    }
                }
                ...
                {
                    Debug.Write("Error 使用函数调用与当前节点不吻合!!");
                    return false;
                }
            }
            else
            {
                if (m_MetaVariable != null)
                {
                    ...
                    if (m_AllowUseSettings.useNotStatic == false && m_MetaVariable.isStatic )
                    {
                        if (frontCNT == ECallNodeType.FunctionInnerVariableName
                            ...
                            || frontCNT == ECallNodeType.Base)
                        {
                            Debug.Write("Error 1 静态调用，不能调用非静态字段!!");
                            return false;
                        }
                    }
                    ...
                    if( this.m_IsArray )
                    {
                        if (m_MetaArrayCallNodeList.Count > 0)   //int[??]
                        {
                            ...
                            if (m_FileMetaCallNode.arrayNodeList.Count > 1 )
                            {
                                ...
                                if (arr2 == 0)
                                {
                                    Debug.Write("Error 数组的第二维长度应该大于0");
                                }
                                count = count * arr2;
                            }
                            if (m_FileMetaCallNode.arrayNodeList.Count > 2 )
                            {
                                ...
                                if (arr3 == 0)
                                {
                                    Debug.Write("Error 数组的第三维长度应该大于0");
                                }
                                count = count * arr3;
                            }
                ...
                {
                    Debug.Write("Error !! 非函数类型!!" + m_FileMetaCallNode.token.ToLexemeAllString());
                }
            }
            return true;
        }
        public bool SetNotStaticVariableOrFunction( MetaBase mb )
        {
            if (mb != null)
            {
                if (m_IsFunction)
                {
                    ...
                    {
                        Debug.Write("Error this.xxx() 不允许使用静态函数");
                        return false;
                    }
                }
                else
                {
                    ...
                    {
                        Debug.Write("Error this.xxx 不允许使用静态");
                        return false;
                    }
                }
                return true;
            ...
            {
                Debug.Write("Error 设置notStatic时，没有找到相应的变量!" + m_Token?.ToLexemeAllString() );
                return false;
            }
        }
        public MetaBase GetFirstNode(string inputname, MetaClass mc)
        {
            ...
            if (m_IsFunction)
            {
                ...
                if( mmf != null )
                {
                    ...
                    {
                        Debug.Write("Error 已定义了其它的形式" + m_CallNodeType.ToString());
                    }
                }
            }
            return retMC;
        ...
        public MetaBase HandleCastFunction(MetaClass mc)
        {
            if (m_MetaTemplateParamsCollection == null)
            {
                Debug.Write("Error 没有Cast<>的使用，请正确使用Cast!!");
                return null;
            }
            else
            {
                ...
                {
                    Debug.Write("Error 没有Cast<ClassName>()的使用，请正确使用Cast!!");
                    return null;
                }
                MetaClass castClass = m_MetaTemplateParamsCollection.metaTemplateParamsList[0].metaClass;
                if (castClass == null)
                {
                    Debug.Write("Error 没有Cast<ClassName>()的使用，没有找到Cast<ClassName> 中的ClassName");
                    return null;
                }
        ...
        public MetaBase GetFunctionOrVariableByOwnerClass(MetaClass mc, string inputname, bool isStatic)
        {
            if (m_IsFunction)
            {
                ...
                return mc.GetMetaMemberFunctionByNameAndInputParamCollect(inputname, m_MetaInputParamCollection, true );                
            }
            else
            {
                var gmb = mc.GetMetaMemberVariableByName(inputname);
                if (gmb == null)
                {
            ...
            if( flag )
            {
                ...
                while (true)
                {
                    ...
                    else if (mcn.callNodeType == ECallNodeType.MemberFunctionName )
                    {
                        ...
                        if (frontNode?.callNodeType == ECallNodeType.ConstValue )
                        {
                            ...
                            if (newmv == null)
                            {
                                ...
                                frontNode.ownerMetaFunctionBlock, frontNode.m_MetaClass, new MetaType(mccm));

                                frontNode.ownerMetaFunctionBlock.AddMetaVariable(newmv);
                            }

                            MetaVisitNode mvn1 = MetaVisitNode.CraeteByNewClass(frontNode.m_MetaClass, mbobs, newmv);
                            m_VisitNodeList.Add(mvn1);


                            mmc = new MetaMethodCall(mcn.m_MetaClass, mcn.m_MetaFunction, mcn.metaInputParamCollection);
                            mmc.SetCallerMetaVariable(newmv);
                            //mmc.SetMethodCallStackType(EMethodCallStackType.CurrentStack);
                        }
                        else
                        {
                            mmc = new MetaMethodCall(mcn.m_MetaClass, mcn.m_MetaFunction, mcn.metaInputParamCollection);
                            newmv = (mcn.m_MetaFunction as MetaMemberFunction).thisMetaVariable;
                            mmc.SetCallerMetaVariable(newmv);
                        }
                    ...
                    else if (mcn.callNodeType == ECallNodeType.NewClass )
                    {
                        if( i == m_CallNodeList.Count )
                        {
                            MetaVisitNode mvn = MetaVisitNode.CraeteByNewClass(mcn.m_MetaClass, mcn.metaBraceStatementsContent);
                            m_VisitNodeList.Add(mvn);

                            if( mcn.m_MetaFunction != null )
                            {
                                MetaMethodCall mmc = new MetaMethodCall(mcn.m_MetaClass, mcn.m_MetaFunction, mcn.metaInputParamCollection);
                    ...
                    {
                        MetaVisitNode mvn = MetaVisitNode.CraeteByNewData( mcn.m_MetaData, mcn.metaBraceStatementsContent);
                        m_VisitNodeList.Add(mvn);
                    }
                    ...
                    {
                        MetaVisitNode mvn = MetaVisitNode.CreateByEnumDefaultValue(mcn.m_MetaEnum, mcn.m_MetaVariable);
                        m_VisitNodeList.Add(mvn);
                    }
