...
`Lz4` 是 LZ4 块压缩 / 解压的静态工具类，位于 `Core` 标准库的 `Text` 命名空间下（`Core/Text/Lz4.sl`），底座是 `ByteBuf`（L0 字节层）。

本体在 C VM 端实现，SL 层无状态纯转发：

```
Lz4.compress / decompress            （SL，Lib/Core/Text/Lz4.sl）
  └─ SystemLz4Compress / Decompress  （syscall，bytebuf_system_method.c §10，栈↔转发）
...
| `static ByteBuf compress( ByteBuf src ) throws` | 压缩 `src` 的可读区，返回**新**缓冲；`src` 索引不变 |
| `static ByteBuf decompress( ByteBuf src ) throws` | 解压 `compress` 的产物，返回**新**缓冲；`src` 索引不变 |
...
入参缓冲已释放（`Released`）时先触发 ByteBuf 的存活守卫（`_ensureLive`）。

## 3. 用法

```sl
# 压缩
var raw = ByteBuf.fromString( "aaaaaaaabbbbcccc" )
...
`test/BaseTest/ByteBufTest.sl` I 组（14 项断言）：bound 边界（空 / 255 / 负数）、可压缩数据收缩与往返、不可压缩数据不缩与往返、空容器 hex 与尺寸、损坏数据（短头部 / 谎报长度 / 释放后调用）抛错。
