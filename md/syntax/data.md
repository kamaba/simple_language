...
- 匿名对象比较前，必须先确认双方是否为匿名 `data`。
- Front 导出类型时，需要带 `dynamic` 标记（运行时可识别匿名 `data`）。
- 运行时加载后应保留该标记，作为比较分支依据。

### 10.2 `data == / !=` 比较规则（最新）

比较顺序应为：

1. 同引用快速路径：若是同一对象引用，直接判等。
2. 若一边是匿名 `data`、另一边是具名/定义 `data`：直接不相等。
3. 若两边都是匿名 `data`：
   - 先比较结构（成员数量一致）；
   - 再比较成员定义（名称、类型、顺序一致）；
   - 最后比较成员值（支持嵌套 `data` 递归比较）。
4. 若两边都是具名/定义 `data`：先类型一致，再比较成员值。

说明：

- 不能只比较底层字节缓冲区；对于引用型成员（如嵌套匿名对象），应按成员值深度比较。

### 10.3 语法层报错前置

- 对 `data x =` 右值缺失（如换行后才出现 `{}` 或被注释打断）的写法，
  应在 Node/File 层尽早报错，不应延后到 Meta 层。

## 11. 整合测试用例规则（DataTest / 集成回归）

建议将以下场景作为固定回归用例集合：

### 11.1 比较语义必测

- 匿名 `data` vs 匿名 `data`（同结构同值）=> `== true`
- 匿名 `data` vs 匿名 `data`（同结构不同值）=> `== false`
- 匿名 `data` vs 具名 `data` => `== false`
- 具名 `data` vs 具名 `data`（同类型同值）=> `== true`
- 具名 `data` vs 具名 `data`（同类型不同值）=> `== false`
- 同引用比较（`a` 与 `a`）=> `== true`
...
- `nested` 成员为匿名 `data` 时：
  - 全量嵌套结构和值相同 => `== true`
  - 任一层级成员值不同 => `== false`
- 混合结构（匿名里嵌具名、具名里嵌匿名）要覆盖至少一组正例和反例。

### 11.3 const / 赋值约束必测

- `const data` 成员改写应失败。
- 匿名 `data` const 子成员改写应失败（若语义启用）。
- 非 const `data`：`new()` 后允许成员重赋值。
- 非 const 静态 `data`：允许静态写入；`const data` 静态写入应失败。

### 11.4 访问与输出必测

- 链式读取（`a.b.c`）在匿名/具名混合层级下正确。
- `toString()/toJson()` 输出包含成员名与成员值。

### 11.5 用例组织建议

- 语法错误用例与运行时语义用例分离。
- 每个比较规则至少包含：正例 1 条 + 反例 1 条。
- 新增规则必须追加回归用例，避免后续重构回退。
