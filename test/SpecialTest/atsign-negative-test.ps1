...
# Usage: powershell -NoProfile -ExecutionPolicy Bypass -File test\SpecialTest\atsign-negative-test.ps1
# Assertions (test/SpecialTest/AtSignNegativeTest.sl, 6 blocks):
#   out/export/AtSignNegativeTest/Logs/Front.txt contains exactly
#   5 x [ProcessAtSignLabelChannelSyntaxError] (LID 20055)
...
# Script lives at <repo>\simple_language\test\SpecialTest\ -> simple_language root is two levels up
$langRoot = (Resolve-Path (Join-Path $PSScriptRoot '..\..')).Path

$proj = Join-Path $langRoot 'test\SpecialTest\AtSignNegativeTest'
...
Write-Host "LID 20055 (channel syntax error): $e55 (expected 5)"
Write-Host "LID 20056 (multiple out-channels): $e56 (expected 1)"

$failed = $false
if ($e55 -ne 5)  { Write-Host '[FAIL] 20055 count mismatch'; $failed = $true }
if ($e56 -ne 1)  { Write-Host '[FAIL] 20056 count mismatch'; $failed = $true }

if ($failed) { exit 1 }
Write-Host '=== AtSign negative test: PASS (all 6 cases reported expected LIDs) ==='
exit 0
