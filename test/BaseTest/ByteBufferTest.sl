# ============================================================================
# test/BaseTest/ByteBufferTest.sl — ByteBuffer L0 字节层冒烟测试
...
ByteBufferTest
{
    static check( string name, bool cond )
    {
        if cond
        {
            global.println( "[ByteBufferTest] " + name + " : OK" )
        }
        else
        {
            global.println( "[ByteBufferTest] " + name + " : FAIL" )
        }
    }

    # ── 异常注入辅助（throws 单动作，供 label/catch 捕获） ──

    static bbReadU8( ByteBuffer buf ) throws
    {
        var v = buf.readU8()
    }

    static bbSetBadReaderIndex( ByteBuffer buf ) throws
    {
        buf.readerIndex = 10
    }

    static bbBadSlice( ByteBuffer buf ) throws
    {
        var v = buf.slice( 3, 5 )
    }
    ...
    static testFactory()
    {
        ...
        var b64 = ByteBuffer( 64 )
        check( "ByteBuffer(64) capacity", b64.capacity == 64 )
        b64.release()

        var hb = ByteBuffer.fromHex( "DeadBeef" )
        ...
        var sb = ByteBuffer.fromString( "ABCD" )
        ...
        four[2] = 3
        four[3] = 4
        var fb = ByteBuffer.fromBytes( four )
        check( "fromBytes roundtrip", fb.toHex() == "01020304" )

        var wb = ByteBuffer.wrap( four )
    ...
    {
        check( "varUintEncodedSize(0)", ByteBuffer.varUintEncodedSize( 0 ) == 1 )
        check( "varUintEncodedSize(127)", ByteBuffer.varUintEncodedSize( 127 ) == 1 )
        check( "varUintEncodedSize(128)", ByteBuffer.varUintEncodedSize( 128 ) == 2 )
        check( "varUintEncodedSize(16383)", ByteBuffer.varUintEncodedSize( 16383 ) == 2 )
        check( "varUintEncodedSize(16384)", ByteBuffer.varUintEncodedSize( 16384 ) == 3 )
    ...
    {
        var src = ByteBuffer.fromHex( "0102030405" )
        var dst = ByteBuffer( 8 )
        src.readBytes( dst, 3 )
    ...
    {
        var b = ByteBuffer.fromHex( "0001020304" )
    ...
    static testCapacity() throws
    {
        ...
        var g = ByteBuffer.fromHex( "ff010203" )
        g.readU8()
        g.discardReadBytes()
    ...
    static lz4Compress( ByteBuffer src ) throws
    {
        var packed = Lz4.compress( src )
        packed.release()
    }

    static lz4Decompress( ByteBuffer src ) throws
    {
    ...
    static fun()
    {
        ...
        testExceptions()
        testLz4()
        global.println( "[ByteBufferTest] all groups done" )
    }
}
