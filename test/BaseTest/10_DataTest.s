...
static data BookCatelogy
{
    a = 20
    # 不允许 Array<Book> bookArray;
}
data Vector3
{
    float x = 0.0f
    ...
    public static Vector3 zero = Vector3(){x = 0, y = 0, z = 0}
    public static Vector3 one = Vector3(){ x = 1.0f, y = 1.0f, z = 1.0f }
}
...
DataTest
{
    root()
    {
        return '{"a":20,"b":30,"message":"ok"}';  #返回传统的json
    }
    string readBook()
    {
    ...
    static Func()
    {
        ...
        SimBook rb1              # 使用data rb 是一个新的SimBook 的实体 等同于
        ...
        data data1 = {a = 20,b = 30, c = [{ a = 1, cc = 3}, { n = 1 }, array(3){1,2,3} ] };  
